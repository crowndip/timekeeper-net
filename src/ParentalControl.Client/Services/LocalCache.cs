...
// enforcement with no cached limits (which would otherwise mean "cannot enforce offline"
// -- i.e. unlimited time until the server is reachable again).
//
// Keyed by normalized username (not the server's user Guid): SystemdSessionMonitor
// creates every session with UserId = Guid.Empty ("server will determine from
// username"), so keying by Guid would merge every local user's state into one bucket
...
public class LocalCache : ILocalCache
{
    ...
    {
        _logger = logger;
        _cacheFilePath = configuration["ParentalControl:CacheFilePath"] ?? "/var/lib/parental-control/cache.json";
    }
    ...
    private void PruneExpiredLocked()
    {
        ...
        // Records are unsynced-only by construction (MarkAsSyncedAsync removes synced ones
        // immediately), but a record that's failed to sync for a full week is treated as
        // unrecoverable rather than growing the cache file forever.
    ...
    private async Task SaveLocked()
    {
        ...
        catch (Exception ex)
        {
            ...
            // running with the in-memory state), but the daemon must never crash over it.
